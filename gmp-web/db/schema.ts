...
export const authEmailCodes = mysqlTable('auth_email_codes', {
...
export const userThirdPartyBindings = mysqlTable('user_third_party_bindings', {
  id: serial('id').primaryKey(),
  ...
  boundAt: datetime('bound_at', { mode: 'string', fsp: 3 }).notNull().default(now),
  lastLoginAt: datetime('last_login_at', { mode: 'string', fsp: 3 }),
})

export const thirdPartyBindSessions = mysqlTable('third_party_bind_sessions', {
  sessionId: varchar('session_id', { length: 191 }).primaryKey(),
  userId: varchar('user_id', { length: 191 }).notNull().references(() => users.userId),
  provider: varchar('provider', { length: 32 }).notNull(),
  state: varchar('state', { length: 191 }).notNull(),
  returnTo: varchar('return_to', { length: 512 }),
  providerUserId: varchar('provider_user_id', { length: 191 }),
  providerDisplayName: varchar('provider_display_name', { length: 255 }),
  providerAvatarUrl: text('provider_avatar_url'),
  callbackError: varchar('callback_error', { length: 500 }),
  expiresAt: datetime('expires_at', { mode: 'string', fsp: 3 }).notNull(),
  confirmedAt: datetime('confirmed_at', { mode: 'string', fsp: 3 }),
  createdAt: datetime('created_at', { mode: 'string', fsp: 3 }).notNull().default(now),
})

export const thirdPartyLoginSessions = mysqlTable('third_party_login_sessions', {
  sessionId: varchar('session_id', { length: 191 }).primaryKey(),
  provider: varchar('provider', { length: 32 }).notNull(),
  state: varchar('state', { length: 191 }).notNull(),
  expectedRole: varchar('expected_role', { length: 32 }),
  returnTo: varchar('return_to', { length: 512 }),
  providerUserId: varchar('provider_user_id', { length: 191 }),
  providerDisplayName: varchar('provider_display_name', { length: 255 }),
  providerAvatarUrl: text('provider_avatar_url'),
  userId: varchar('user_id', { length: 191 }).references(() => users.userId),
  callbackError: varchar('callback_error', { length: 500 }),
  expiresAt: datetime('expires_at', { mode: 'string', fsp: 3 }).notNull(),
...
export const simulationProjectProgress = mysqlTable('simulation_project_progress', {
  userId: varchar('user_id', { length: 191 }).notNull().references(() => users.userId),
  projectId: int('project_id').notNull(),
  medal: varchar('medal', { length: 32 }).notNull(),
  bestScore: int('best_score').notNull().default(0),
  storyScore: int('story_score').notNull().default(0),
  bossAccuracy: int('boss_accuracy').notNull().default(0),
  creditHours: double('credit_hours').notNull().default(0),
...
export const aiChatSessions = mysqlTable('ai_chat_sessions', {
  sessionId: varchar('session_id', { length: 191 }).primaryKey(),
  userId: varchar('user_id', { length: 191 }).notNull().references(() => users.userId),
  audience: varchar('audience', { length: 32 }).notNull().default('student'),
...
export const aiChatMessages = mysqlTable('ai_chat_messages', {
  id: serial('id').primaryKey(),
  sessionId: varchar('session_id', { length: 191 }).notNull().references(() => aiChatSessions.sessionId, { onDelete: 'cascade' }),
  role: varchar('role', { length: 32 }).notNull(),
  content: text('content').notNull(),
  sources: text('sources'),
  criticTriggered: boolean('critic_triggered').notNull().default(false),
  createdAt: datetime('created_at', { mode: 'string', fsp: 3 }).notNull().default(now),
})

export const aiFeedbackLog = mysqlTable('ai_feedback_log', {
  id: serial('id').primaryKey(),
  userId: varchar('user_id', { length: 191 }).notNull().references(() => users.userId),
  sessionId: varchar('session_id', { length: 191 }).references(() => aiChatSessions.sessionId, { onDelete: 'set null' }),
  messageId: bigint('message_id', { mode: 'number', unsigned: true }).references(() => aiChatMessages.id, { onDelete: 'set null' }),
  messageRole: varchar('message_role', { length: 32 }).notNull().default('assistant'),
...
export const courseAssignmentSubmissions = mysqlTable('course_assignment_submissions', {
  ...
  gradedBy: varchar('graded_by', { length: 32 }).notNull().default('ai'),
})
