...
export default function CourseHomePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/course/overview', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.ok ? r.json() : null),
      fetch('/api/course/leaderboard', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.ok ? r.json() : null),
    ])
      .then(([ov, l]) => { setData(ov); setLb(l) })
  ...
  const progressPct = summary.totalChapters > 0
    ...
    : 0
  const hourPct = summary.totalMaxHours > 0
    ? Math.round(summary.totalEarnedHours / summary.totalMaxHours * 100)
    : 0
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 24, position: 'relative' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, maxWidth: 600 }}>
              {[
                ...
                { label: '累计课时分', value: summary.totalEarnedHours, total: summary.totalMaxHours, suffix: '学时', icon: Trophy },
                { label: '本周学习', value: Math.round(summary.weekStudyMinutes / 60 * 10) / 10, suffix: '小时', icon: Clock },
                { label: '总学习时长', value: Math.round(summary.totalStudyMinutes / 60 * 10) / 10, suffix: '小时', icon: TrendingUp },
          ...
          <div style={{ display: 'flex', gap: 18, alignItems: 'center', flexShrink: 0 }}>
            ...
            <DonutProgress pct={hourPct} label="课时分进度" size={108} thick={9} accent="#ffd76b" track="rgba(255,255,255,0.15)" />
          </div>
        </div>
      </div>
            ...
            return (
              <div key={u.userId} style={{
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <p style={{ margin: 0, fontSize: 9, color: '#9aacb6' }}>学时</p>
                </div>
              </div>
            )
          })}
...
function LoadingState() {
  return (
    <div style={{ padding: 60, textAlign: 'center', color: '#7a96a4' }}>
      ...
      <p style={{ margin: 0, fontSize: 13 }}>加载课程数据中…</p>
    </div>
  )
}
