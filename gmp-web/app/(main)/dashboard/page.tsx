...
const TOTAL_CREDIT_TARGET =
  COURSE_CREDIT_RULES.courseLearningRegular + SIMULATION_CREDIT_TARGET + COURSE_CREDIT_RULES.courseFinalTest
...
type AchievementCategory = '学习' | '连续' | '实训' | '综合'
...
function achievementProgress(badge: AchievementBadge) {
  if (badge.target <= 0) return 0
  return Math.min(100, Math.max(0, Math.round((badge.current / badge.target) * 100)))
}

function formatAchievementValue(value: number) {
  return Number.isInteger(value) ? String(value) : value.toFixed(1)
}

function AchievementMedallion({ badge, size = 80 }: { badge: AchievementBadge; size?: number }) {
  const Icon = badge.Icon
  const gradientId = `achievement-${badge.key}-${size}`
  const stroke = badge.unlocked ? 'rgba(255,255,255,0.86)' : 'rgba(132,146,154,0.55)'
  const color = badge.unlocked ? badge.color : '#9ca8ae'
  return (
    <div style={{
      width: size,
      height: size,
      ...
      filter: badge.unlocked
        ? `drop-shadow(0 10px 16px ${badge.color}36)`
        : 'grayscale(1) saturate(0.08) opacity(0.72) drop-shadow(0 7px 10px rgba(71,85,105,0.18))',
    }}>
      <svg width={size} height={size} viewBox="0 0 80 80" aria-hidden="true" style={{ position: 'absolute', inset: 0 }}>
        <defs>
          <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor={badge.unlocked ? badge.color : '#d5dade'} stopOpacity={badge.unlocked ? 0.95 : 0.86} />
            <stop offset="100%" stopColor={badge.unlocked ? '#f6d078' : '#aeb7bc'} stopOpacity={badge.unlocked ? 0.72 : 0.78} />
          </linearGradient>
        </defs>
        <polygon points="40,5 69,22 69,58 40,75 11,58 11,22" fill={`url(#${gradientId})`} />
        <polygon points="40,10 64,25 64,55 40,70 16,55 16,25" fill="rgba(255,255,255,0.18)" stroke={stroke} strokeWidth="2" />
        <path d="M20 57 L40 69 L60 57 L60 63 L40 76 L20 63 Z" fill={badge.unlocked ? 'rgba(127,88,26,0.28)' : 'rgba(107,114,128,0.22)'} />
      </svg>
      <div style={{
        position: 'relative',
        zIndex: 1,
        ...
        alignItems: 'center',
        justifyContent: 'center',
        background: badge.unlocked ? 'rgba(255,255,255,0.82)' : 'rgba(255,255,255,0.58)',
        color,
        border: `1px solid ${badge.unlocked ? 'rgba(255,255,255,0.8)' : 'rgba(132,146,154,0.36)'}`,
      }}>
        <Icon size={Math.round(size * 0.22)} strokeWidth={1.9} />
      </div>
    </div>
  )
}
...
function scopedSimulationKey(baseKey: string, userId?: string | null) {
  const resolvedUserId = userId ?? localStorage.getItem('userId')
  return resolvedUserId ? `${baseKey}:${resolvedUserId}` : baseKey
}

function isCompletedSimulationMedal(value: unknown): value is Exclude<ProjectMedal, 'none'> {
  return value === 'bronze' || value === 'silver' || value === 'gold'
}

function summarizeSimulationCreditHours(progress: Record<string, SimulationProgressEntry>) {
  const earned = Object.entries(progress).reduce((sum, [key, item]) => {
    const projectId = Number(key)
    const medal = isCompletedSimulationMedal(item.medal)
      ? item.medal
      : Number(item.creditHours) > 0 ? 'bronze' : 'none'
    return sum + baseCreditForProjectMedal(projectId, medal) + medalBonusCreditForProject(projectId, medal)
  }, 0)
  return Number(Math.min(earned, SIMULATION_CREDIT_TARGET).toFixed(1))
}

function readSimulationAssets(userId?: string | null) {
  const walletKey = scopedSimulationKey(SIMULATION_WALLET_KEY, userId)
  const progressKey = scopedSimulationKey(SIMULATION_PROGRESS_KEY, userId)
  const hpKey = scopedSimulationKey(SIMULATION_HP_KEY, userId)
  const storedWallet = localStorage.getItem(walletKey)
  const storedProgress = localStorage.getItem(progressKey)
  const storedHpRaw = localStorage.getItem(hpKey)
  ...
  if (storedProgress) {
    try {
      ...
      assets.hasProgress = Object.keys(progress).length > 0
      assets.creditHours = summarizeSimulationCreditHours(progress)
  ...
  if (Number.isFinite(storedHp)) {
    assets.hp = clampHp(storedHp)
...
export default function DashboardPage() {
  ...
  const [courseSummary, setCourseSummary] = useState<DashboardCourseSummary | null>(null)
  ...
  useEffect(() => {
    ...
    fetch('/api/course/overview', { headers: { Authorization: `Bearer ${t}` } })
      .then(r => r.ok ? r.json() : null)
      .then(data => setCourseSummary(data?.summary ?? null))
      .catch(() => setCourseSummary(null))
    return () => {
  ...
  // ── 成就徽章 ─────────────────────────────────────────────────────────────────
  const completedCourseCount = courseSummary?.completedChapters ?? 0
  const totalCourseCount = courseSummary?.totalChapters ?? 0
  const courseEarnedCredits = courseSummary?.totalEarnedCredits ?? 0
  const totalEarnedCredits = courseEarnedCredits + assets.creditHours
  const trophyCount = assetsReady ? assets.trophies : 0

  const ALL_BADGES: AchievementBadge[] = [
    { key: 'beginner', label: '初学者', sub: `完成${completedCourseCount}/10课程`, detail: '完成 10 个课程学习节点，建立 GMP 基础框架。', category: '学习', color: '#1d6f78', unlocked: completedCourseCount >= 10, current: completedCourseCount, target: 10, unit: '课程', Icon: BookOpenCheck },
    { key: 'diligent', label: '勤奋学习', sub: `连续学习${streak}/7天`, detail: '连续学习 7 天，形成稳定的学习节奏。', category: '连续', color: '#0891b2', unlocked: streak >= 7, current: streak, target: 7, unit: '天', Icon: Flame },
    { key: 'explorer', label: '知识探索者', sub: `完成${completedCourseCount}/50课程`, detail: '累计完成 50 个课程学习节点，扩展法规和质量体系理解。', category: '学习', color: '#7c3aed', unlocked: completedCourseCount >= 50, current: completedCourseCount, target: 50, unit: '课程', Icon: Network },
    { key: 'expert', label: '实训达人', sub: `获得${trophyCount}/20勋章`, detail: '在实训仿真中获得 20 枚勋章，具备持续通关能力。', category: '实训', color: '#b87808', unlocked: trophyCount >= 20, current: trophyCount, target: 20, unit: '勋章', Icon: Trophy },
    { key: 'first-trophy', label: '首枚勋章', sub: `获得${trophyCount}/1勋章`, detail: '完成任意一个实训项目并获得第一枚通关勋章。', category: '实训', color: '#c2410c', unlocked: trophyCount >= 1, current: trophyCount, target: 1, unit: '勋章', Icon: Medal },
    { key: 'simulation-credit', label: '课时收集者', sub: `实训课时${formatAchievementValue(assets.creditHours)}/${SIMULATION_CREDIT_TARGET}`, detail: '通过实训项目累计获得指定课时分。', category: '实训', color: '#0f766e', unlocked: assets.creditHours >= SIMULATION_CREDIT_TARGET, current: assets.creditHours, target: SIMULATION_CREDIT_TARGET, unit: '课时分', Icon: GraduationCap },
    { key: 'credit-master', label: '学分统筹者', sub: `总课时${formatAchievementValue(totalEarnedCredits)}/${TOTAL_CREDIT_TARGET}`, detail: '课程学习与实训课时累计达到课程总目标。', category: '综合', color: '#2563eb', unlocked: totalEarnedCredits >= TOTAL_CREDIT_TARGET, current: totalEarnedCredits, target: TOTAL_CREDIT_TARGET, unit: '课时分', Icon: Target },
    { key: 'rank-up', label: 'GMP进阶者', sub: `等级${gs?.rankLevel ?? 1}/3`, detail: '等级达到 Lv.3，说明学习、练习与打卡已经开始形成积累。', category: '综合', color: '#4f46e5', unlocked: (gs?.rankLevel ?? 1) >= 3, current: gs?.rankLevel ?? 1, target: 3, unit: '级', Icon: Award },
    { key: 'xp-collector', label: 'XP积累者', sub: `XP ${gs?.xp ?? 0}/1000`, detail: '累计获得 1000 XP，持续参与课程、练习和打卡。', category: '综合', color: '#059669', unlocked: (gs?.xp ?? 0) >= 1000, current: gs?.xp ?? 0, target: 1000, unit: 'XP', Icon: Star },
    { key: 'steady', label: '稳态学习者', sub: `最长连续${gs?.maxStreak ?? 0}/14天`, detail: '历史最长连续学习达到 14 天。', category: '连续', color: '#ea580c', unlocked: (gs?.maxStreak ?? 0) >= 14, current: gs?.maxStreak ?? 0, target: 14, unit: '天', Icon: Clock },
    { key: 'quality-guard', label: '质量守护者', sub: `实训勋章${trophyCount}/5`, detail: '获得 5 枚实训勋章，完成多个质量风险场景。', category: '实训', color: '#be123c', unlocked: trophyCount >= 5, current: trophyCount, target: 5, unit: '勋章', Icon: ShieldCheck },
    { key: 'all-rounder', label: '综合精英', sub: `${completedCourseCount}/10课 · ${streak}/7天 · ${trophyCount}/5章`, detail: '同时达成课程、连续学习和实训三类核心目标。', category: '综合', color: '#9333ea', unlocked: completedCourseCount >= 10 && streak >= 7 && trophyCount >= 5, current: [completedCourseCount / 10, streak / 7, trophyCount / 5].filter(Number.isFinite).reduce((sum, value) => sum + Math.min(1, value), 0), target: 3, unit: '项', Icon: CheckCircle },
  ]
  const BADGES = ALL_BADGES.slice(0, 4)
  const unlockedBadgeCount = ALL_BADGES.filter(badge => badge.unlocked).length
  ...
  const courseProgress = Math.min(100, Math.max(0, Math.round(courseSummary?.coursewareProgressPct ?? 0)))
        ...
        <div style={{ ...PANEL, minHeight: 270, padding: '20px 26px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '48% 52%', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
              {([
                { Icon: BookOpen, label: '已学课程', value: courseSummary ? `${completedCourseCount}/${totalCourseCount}` : '—', unit: '节', iconColor: '#0f766e', iconBg: 'rgba(15,118,110,0.10)' },
                { Icon: Clock, label: '总课时分', value: courseSummary || assetsReady ? totalEarnedCredits.toFixed(1) : '—', unit: `/ ${TOTAL_CREDIT_TARGET} 分`, iconColor: '#0ea5e9', iconBg: 'rgba(14,165,233,0.10)' },
                { Icon: Award, label: '已获得证书', value: assetsReady ? `${trophyCount}` : '—', unit: '个', iconColor: '#d97706', iconBg: 'rgba(217,119,6,0.10)' },
              ] as const).map(({ Icon, label, value, unit, iconColor, iconBg }) => (
                <div key={label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 0', borderTop: 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                    ...
                    <span style={{ fontSize: 13, color: '#2d4a58', fontWeight: 700 }}>{label}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                    <strong style={{ fontSize: 20, color: '#0f2f3f', fontVariantNumeric: 'tabular-nums', fontWeight: 800 }}>{value}</strong>
                    <span style={{ fontSize: 11, color: '#9ba8b0' }}>{unit}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ flex: 1, padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ marginTop: 6, height: 4, borderRadius: 999, background: 'rgba(16,185,129,0.12)', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${assetsReady ? assets.hp : 0}%`, background: 'linear-gradient(90deg,#10b981,#6ee7b7)', borderRadius: 999 }} />
              </div>
            </div>
          </div>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.35fr) minmax(250px,0.7fr) minmax(380px,1fr)', gap: 16, alignItems: 'stretch' }}>
        ...
        <div style={{ ...PANEL, minHeight: 205, padding: '16px 20px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
            <svg width="100%" viewBox="0 0 730 100" style={{ overflow: 'visible' }} preserveAspectRatio="xMidYMid meet">
              <defs>
                <linearGradient id="pathGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" stopColor="#5dd8d0" />
                  <stop offset="100%" stopColor="#3aada5" />
                </linearGradient>
                <linearGradient id="planeTop" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#93c5fd" />
                  <stop offset="100%" stopColor="#0ea5e9" />
                </linearGradient>
                <linearGradient id="planeBot" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" />
                  <stop offset="100%" stopColor="#0c4a6e" />
                </linearGradient>
                <filter id="planeShadow" x="-20%" y="-20%" width="140%" height="140%">
                  <feDropShadow dx="1" dy="2" stdDeviation="1.5" floodColor="#0369a1" floodOpacity="0.35" />
                </filter>
              </defs>
              {(() => {
                const PTS = [
                ...
                const MIDS = [
                  { x: 110, y: 61, done: true },
                  { x: 246, y: 41, done: true },
                  { x: 380, y: 44, done: false },
                  { x: 509, y: 62, done: false },
                  { x: 625, y: 57, done: false },
                ] as const
                ...
                return (
                  <>
                    <circle cx={10} cy={66} r={3} fill="#cdd9dd" />
                    <line x1={14} y1={66} x2={44} y2={66} stroke="#dce6ea" strokeWidth={2.8} strokeLinecap="round" />
                    ...
                    {MIDS.map((m, i) => (
                      <circle key={`mid-${i}`} cx={m.x} cy={m.y} r={3.5}
                        fill={m.done ? '#5dd8d0' : '#c8d7dc'}
                        stroke={m.done ? '#3aada5' : '#b5c6cc'} strokeWidth={1} />
                    ))}
                    <g transform="translate(688, 33)" filter="url(#planeShadow)">
                      <path d="M 2 10 L 26 5 L 9 0 Z" fill="url(#planeTop)" />
                      <path d="M 2 10 L 26 5 L 9 20 Z" fill="url(#planeBot)" />
                      <line x1={9} y1={0} x2={26} y2={5} stroke="rgba(255,255,255,0.55)" strokeWidth={0.9} />
                      <line x1={9} y1={20} x2={26} y2={5} stroke="rgba(3,69,120,0.28)" strokeWidth={0.7} />
                      <line x1={2} y1={10} x2={26} y2={5} stroke="rgba(255,255,255,0.65)" strokeWidth={0.75} />
                      <path d="M 2 10 L 0 7 L 5 10 Z" fill="#0369a1" opacity={0.55} />
                    </g>
                    {/* 当前节点上升小箭头 */}
            ...
            <svg width={186} height={170} viewBox="0 0 220 200" style={{ overflow: 'visible' }}>
              <defs>
                <radialGradient id="radarGradient" cx="50%" cy="50%" r="55%">
                  <stop offset="0%" stopColor="rgba(20,184,166,0.60)" />
                  <stop offset="100%" stopColor="rgba(15,118,110,0.38)" />
                </radialGradient>
              </defs>
        ...
        <div style={{ ...PANEL, minHeight: 205, padding: '16px 22px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ height: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button type="button" onClick={() => setAchievementsOpen(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#0f766e', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
              查看全部成就 <ArrowRight size={13} />
            </button>
          </div>
          <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, alignContent: 'center' }}>
            {BADGES.map(badge => (
              <div key={badge.key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7 }}>
                <AchievementMedallion badge={badge} />
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: badge.unlocked ? '#0f2f3f' : '#7d8f99' }}>{badge.label}</div>
                  <div style={{ fontSize: 11, color: badge.unlocked ? '#6b8794' : '#9aaab2', marginTop: 2, lineHeight: 1.3 }}>{badge.sub}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        >
          <div
            ...
            style={{
              ...
              background: '#f8fbfc',
              borderRadius: 12,
              border: '1px solid rgba(210,226,231,0.96)',
              boxShadow: '0 28px 70px rgba(15,35,45,0.26)',
              display: 'flex',
              flexDirection: 'column',
            }}
          >
            <div style={{ padding: '18px 22px', borderBottom: '1px solid rgba(34,73,84,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, background: '#fff' }}>
              <div>
                <p style={{ margin: '0 0 4px', color: '#0f766e', fontSize: 12, fontWeight: 800, letterSpacing: 0 }}>ACHIEVEMENT COLLECTION</p>
                <h2 id="achievements-title" style={{ margin: 0, color: '#0f2f3f', fontSize: 20, fontWeight: 800 }}>全部成就徽章</h2>
              </div>
              <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ minWidth: 120, padding: '8px 12px', borderRadius: 8, background: 'rgba(15,118,110,0.08)', border: '1px solid rgba(15,118,110,0.18)', color: '#0f766e', textAlign: 'center' }}>
                  <strong style={{ display: 'block', fontSize: 20, lineHeight: 1 }}>{unlockedBadgeCount}/{ALL_BADGES.length}</strong>
                  <span style={{ fontSize: 11 }}>已点亮</span>
                </div>
                <button
                  type="button"
                  onClick={() => setAchievementsOpen(false)}
                  aria-label="关闭全部成就"
                  title="关闭"
                  style={{ width: 34, height: 34, borderRadius: 8, border: '1px solid rgba(31,71,92,0.14)', background: '#fff', color: '#6b8a98', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
                >
                  <X size={18} />
                </button>
              </div>
            ...
            <div style={{ padding: 22, overflowY: 'auto' }}>
              {(['学习', '连续', '实训', '综合'] as AchievementCategory[]).map(category => {
                const group = ALL_BADGES.filter(badge => badge.category === category)
                return (
                  <section key={category} style={{ marginBottom: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                      <h3 style={{ margin: 0, color: '#183b4b', fontSize: 15, fontWeight: 800 }}>{category}成就</h3>
                      <span style={{ color: '#7a95a0', fontSize: 12 }}>{group.filter(badge => badge.unlocked).length}/{group.length}</span>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: 10 }}>
                      {group.map(badge => {
                        const pct = achievementProgress(badge)
                        return (
                          <article key={badge.key} style={{
                            borderRadius: 8,
                            border: `1px solid ${badge.unlocked ? `${badge.color}33` : 'rgba(31,71,92,0.12)'}`,
                            background: badge.unlocked ? '#fff' : 'rgba(255,255,255,0.68)',
                            padding: 14,
                            display: 'grid',
                            ...
                            <AchievementMedallion badge={badge} size={72} />
                            <div style={{ minWidth: 0 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                                <strong style={{ color: badge.unlocked ? '#0f2f3f' : '#6b7d86', fontSize: 14 }}>{badge.label}</strong>
                                <span style={{ padding: '2px 7px', borderRadius: 999, fontSize: 10, fontWeight: 800, color: badge.unlocked ? '#0f766e' : '#7a95a0', background: badge.unlocked ? 'rgba(15,118,110,0.1)' : 'rgba(122,149,160,0.12)' }}>
                                  {badge.unlocked ? '已点亮' : '未解锁'}
                                </span>
                              </div>
                              <p style={{ margin: '0 0 8px', color: '#6b8794', fontSize: 12, lineHeight: 1.5 }}>{badge.detail}</p>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <div style={{ flex: 1, height: 6, borderRadius: 999, background: 'rgba(31,71,92,0.08)', overflow: 'hidden' }}>
                                  <div style={{ width: `${pct}%`, height: '100%', borderRadius: 999, background: badge.unlocked ? `linear-gradient(90deg,${badge.color},#f2c45d)` : 'linear-gradient(90deg,#9fb0b7,#c7d0d4)' }} />
                                </div>
                                <span style={{ minWidth: 76, textAlign: 'right', color: '#5d7580', fontSize: 11, fontWeight: 700 }}>
                                  {formatAchievementValue(badge.current)}/{formatAchievementValue(badge.target)}{badge.unit}
                                </span>
                              </div>
                            </div>
                          </article>
