...
export default function GraphPanel({
  ...
  const [stats,   setStats]   = useState<{ mastered: number; learning: number; weak: number; untested: number; issueTotal?: number; issueReviewed?: number } | null>(null)
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      try {
        // 掌握度模式：同时拉知识图谱 + 个人掌握数据；章节页可直接传入局部图谱数据。
        const graphType = isMastery ? 'knowledge' : type
        const fetches: Promise<Response>[] = dataOverride
          ? []
        ...
        const data: GraphData = dataOverride ?? await responses[0].json()
        const masteryMap: Record<string, MasteryEntry> = isMastery
          ? (await responses[dataOverride ? 0 : 1].json()).masteryMap
          : {}
        ...
        if (isIssue) {
          const issueTotal = data.nodes.length
          const issueReviewed = data.nodes.filter(node => node.reviewed).length
          setStats({ mastered: 0, learning: 0, weak: 0, untested: 0, issueTotal, issueReviewed })
        }
        ...
        const coloredNodes = data.nodes.map(node => {
          if (isIssue) {
            return {
              ...node,
              category: node.reviewed ? 1 : 0,
              symbolSize: Math.max(16, node.symbolSize),
              itemStyle: {
                color: node.reviewed ? ISSUE_COLORS.reviewed : ISSUE_COLORS.unreviewed,
        ...
        const coloredCategories = isMastery
          ...
          : isIssue
            ? [
                { name: '未复盘错题', itemStyle: { color: ISSUE_COLORS.unreviewed } },
                { name: '已复盘错题', itemStyle: { color: ISSUE_COLORS.reviewed } },
              ]
        ...
        const option = {
          ...
          tooltip: {
            ...
            formatter: (params: any) => {
              ...
              if (isIssue) {
                return `<div style="max-width:280px;font-size:13px;line-height:1.7">
                  <b>${params.data.fullStem || params.data.name}</b><br/>
                  <span style="color:#888">${params.data.project}</span><br/>
                  <span style="color:#aaa;font-size:12px">${params.data.questionType || '题目'} · ${params.data.task || ''}</span><br/>
                  <span style="color:${params.data.reviewed ? ISSUE_COLORS.reviewed : ISSUE_COLORS.unreviewed};font-weight:700">
                    错 ${params.data.wrongCount || 0} 次 · 对 ${params.data.correctCount || 0} 次 · ${params.data.reviewed ? '已复盘' : '未复盘'}
                  </span>
                </div>`
              }
          ...
          series: [
            {
              ...
              draggable: true,
              force: {
                repulsion: (type === 'ability') ? 200 : isIssue ? 160 : 80,
                edgeLength: (type === 'ability') ? 120 : isIssue ? 95 : 60,
                gravity: 0.08,
                layoutAnimation: true,
              },
              label: {
                show: type === 'ability' || isIssue,
                position: 'bottom',
                fontSize: 11,
                color: '#333',
                formatter: (params: any) => {
              ...
              lineStyle: { color: 'source', opacity: isMastery ? 0.2 : isIssue ? 0.28 : 0.4, width: 1.5, curveness: 0.2 },
              itemStyle: isMastery
  ...
  }, [type, token, isMastery, isIssue, dataOverride])
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%', minHeight }}>
      {loading && (
        <div style={{
          ...
          <span style={{ fontSize: 13 }}>{isMastery ? '正在加载个人进度…' : isIssue ? '正在加载问题图谱…' : '正在加载图谱…'}</span>
      ...
      {isIssue && !loading && !error && stats && (
        <div style={{
          position: 'absolute', top: 8, left: 8, zIndex: 10,
          display: 'flex', gap: 6,
        }}>
          {[
            { label: '未复盘', count: Math.max(0, (stats.issueTotal ?? 0) - (stats.issueReviewed ?? 0)), color: ISSUE_COLORS.unreviewed },
            { label: '已复盘', count: stats.issueReviewed ?? 0, color: ISSUE_COLORS.reviewed },
          ].map(s => (
            <div key={s.label} style={{
              display: 'flex', alignItems: 'center', gap: 5,
              padding: '4px 10px', borderRadius: 20,
      ...
      {isIssue && !loading && !error && stats && (stats.issueTotal ?? 0) === 0 && (
        <div style={{
          position: 'absolute',
          inset: 0,
          zIndex: 5,
          ...
          <div style={{
            ...
            border: '1px dashed rgba(188,91,87,0.24)',
            color: '#6b8a98',
            fontSize: 13,
            boxShadow: '0 10px 24px rgba(24,59,75,0.08)',
          }}>
            暂无错题记录，完成练习或章节测验后会自动生成问题图谱。
          </div>
        </div>
      )}
