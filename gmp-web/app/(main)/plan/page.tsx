...
interface PlanData {
  ...
  generated_by?: 'rules' | 'ai+rules'
  dynamic_profile?: { generated_at?: string } | null
  created_at?: string
}
...
export default function PlanPage() {
  ...
  const generatedLabel = planData.generated_by === 'ai+rules' ? 'AI+规则动态导学' : '规则动态导学'
  const evidenceProjectCount = plan.filter(item => item.evidence?.length).length
  const maxAdaptiveScore = plan.reduce((max, item) => Math.max(max, item.adaptive_score ?? 0), 0)
  const dynamicRefreshText = planData.dynamic_profile?.generated_at
    ? new Date(planData.dynamic_profile.generated_at).toLocaleString('zh-CN', { hour12: false })
    : '本次访问'
    ...
    <div className="plan-page">
      ...
      <header className="plan-header">
        <div>
          ...
          <h1>个性化学习方案</h1>
          <p>基于前测、知识点掌握度、章节测验、作业和学习投入动态生成，按“练习、课程、实训”推进。</p>
        </div>
        ...
        <div className="score-panel">
          ...
          <div>
            ...
            <div className="meta-row">
              ...
              <span><Sparkles size={14} />{generatedLabel}</span>
            </div>
          </div>
        </div>
      ...
      <div className="content-grid">
        ...
        <aside style={PANEL} className="plan-section">
          ...
          <div className="weak-list">
            {(scheme.weak_items.length ? scheme.weak_items : prioritySort(plan)).slice(0, 3).map(item => {
                ...
                <div key={item.project_name} className="weak-item">
                  ...
                  {item.evidence?.length ? (
                    <div className="evidence-list">
                      {item.evidence.slice(0, 3).map(evidence => <em key={evidence}>{evidence}</em>)}
                    </div>
                  ) : null}
                </div>
              )
      ...
      <div className="priority-list">
        {items.length === 0 ? (
          ...
          <div key={item.project_name} className="priority-item">
            ...
            <p>{item.reason}</p>
            <small>
              {item.adaptive_score !== undefined ? `动态评分 ${item.adaptive_score}` : item.total > 0 ? `错 ${item.wrong} / 答 ${item.total}` : '前测未涉及'}
            </small>
            {item.evidence?.length ? (
...
.signal-card {
  ...
  border-radius: 10px;
  background: rgba(255,255,255,0.92);
  box-shadow: 0 10px 28px rgba(29,53,74,0.06);
}
...
.week-item {
  display: grid;
  grid-template-columns: 30px minmax(88px, 0.42fr) minmax(0, 1fr);
  gap: 8px;
  align-items: center;
  padding: 9px 10px;
  border: 1px solid rgba(31,71,92,0.08);
