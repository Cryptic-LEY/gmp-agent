...
import { useEffect, useRef, useState, type ChangeEvent, type CSSProperties, type Dispatch, type SetStateAction } from 'react'
import {
  BookOpen,
  Building2,
  Calendar,
...
export default function RoleProfileCenter({
  ...
  const [wechatMsg, setWechatMsg] = useState<{ ok: boolean; text: string } | null>(null)
  ...
  async function loadWechatBinding() {
    ...
    try {
      ...
      setWechatBound(Boolean(wechat?.bound))
    } catch {
      setWechatMsg({ ok: false, text: '微信绑定状态读取失败' })
    }
  }
  ...
  async function handleWechatBind() {
    const token = localStorage.getItem('token')
    if (!token) {
      setWechatMsg({ ok: false, text: '请先登录后再绑定微信' })
      return
    }
    ...
    try {
      ...
      const data = await response.json()
      if (!response.ok) {
        setWechatMsg({ ok: false, text: data.error || '微信绑定发起失败' })
        return
      }
      setWechatMsg({ ok: true, text: '正在打开微信扫码授权页...' })
      window.location.href = data.authUrl
    } catch (error) {
      setWechatMsg({ ok: false, text: error instanceof Error ? error.message : '微信绑定发起失败' })
    } finally {
  ...
  async function handleWechatUnbind() {
    ...
    try {
      ...
      if (!response.ok) {
        setWechatMsg({ ok: false, text: data.error || '微信解绑失败' })
        return
      }
      setWechatBound(false)
      setWechatMsg({ ok: true, text: '已解绑微信账号' })
    } catch (error) {
      setWechatMsg({ ok: false, text: error instanceof Error ? error.message : '微信解绑失败' })
    } finally {
      setWechatLoading(false)
    }
  }
  ...
  async function handlePasswordChange() {
    ...
    if (!isStrongPassword(pwForm.newPw)) {
      setPwMsg({ ok: false, text: PASSWORD_POLICY_MESSAGE })
      return
    }
          ...
          {activeTab === 'apps' && (
            <div style={{ maxWidth: 520 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderRadius: 10, border: `1px solid ${wechatBound ? 'rgba(29,111,120,0.2)' : '#dde3e8'}`, background: wechatBound ? 'rgba(29,111,120,0.04)' : '#fafafa', marginBottom: 20 }}>
                ...
                <button
                  onClick={wechatBound ? handleWechatUnbind : handleWechatBind}
                  disabled={wechatLoading}
                  style={{ padding: '8px 20px', borderRadius: 6, fontSize: 13, fontWeight: 600, border: wechatBound ? '1px solid #dde3e8' : 'none', background: wechatBound ? '#fff' : '#07c160', color: wechatBound ? '#6b8a98' : '#fff', cursor: wechatLoading ? 'wait' : 'pointer', opacity: wechatLoading ? 0.72 : 1 }}
                >
                  {wechatLoading ? '处理中...' : wechatBound ? '解绑' : '立即绑定'}
                </button>
              </div>

              {wechatMsg && (
                <div style={{ padding: '10px 12px', borderRadius: 8, background: wechatMsg.ok ? 'rgba(7,193,96,0.08)' : '#fef2f2', border: `1px solid ${wechatMsg.ok ? 'rgba(7,193,96,0.2)' : '#fecaca'}`, color: wechatMsg.ok ? '#087f43' : '#dc2626', fontSize: 12, marginBottom: 16 }}>
                  {wechatMsg.text}
                </div>
              )}
