...
interface PendingStudentRegistration {
  email: string
  registrationToken: string
  displayName: string
  role: 'student'
  profile?: Partial<RegInfo>
  eduLevel?: string
  major?: string
}
...
function emptyRegInfo(): RegInfo {
  return { realName: '', school: '', className: '', teacherUserId: '', studentId: '', idCard: '', phone: '' }
}

function normalizeRegInfo(value?: Partial<RegInfo>): RegInfo {
  return { ...emptyRegInfo(), ...(value ?? {}) }
}
...
export default function OnboardingPage() {
  ...
  // Step 0 — 注册信息
  const [regInfo, setRegInfo] = useState<RegInfo>(emptyRegInfo)
  const [regEmail, setRegEmail]       = useState('')
  const [regDisplay, setRegDisplay]   = useState('')
  const [regSaving, setRegSaving]     = useState(false)
  const [regError, setRegError]       = useState('')
  const [teacherOptions, setTeacherOptions] = useState<TeacherOption[]>([])
  const [pendingRegistration, setPendingRegistration] = useState<PendingStudentRegistration | null>(null)
  ...
  function updatePendingRegistration(patch: Partial<PendingStudentRegistration>) {
    if (!pendingRegistration) return null
    const next = { ...pendingRegistration, ...patch }
    setPendingRegistration(next)
    sessionStorage.setItem(PENDING_STUDENT_REGISTRATION_KEY, JSON.stringify(next))
    return next
  }
  ...
  useEffect(() => {
    ...
    const pendingText = sessionStorage.getItem(PENDING_STUDENT_REGISTRATION_KEY)

    if (token && localStorage.getItem('onboarding_done')) {
      router.push('/dashboard')
      return
    }

    if (!token && !pendingText) {
      router.push('/login')
      return
    }
    ...
    if (!token && pendingText) {
      try {
        const pending = JSON.parse(pendingText) as PendingStudentRegistration
        if (!pending.email || !pending.registrationToken || !pending.displayName) {
          router.push('/login')
          return
        }
        setPendingRegistration(pending)
        setRegEmail(pending.email)
        setRegDisplay(pending.displayName)
        setRegInfo(normalizeRegInfo(pending.profile))
        setEduLevel(pending.eduLevel ?? '')
        setMajor(pending.major ?? '')
      } catch {
    ...
    const teacherHeaders = token ? { Authorization: `Bearer ${token}` } : undefined
    fetch('/api/teachers', teacherHeaders ? { headers: teacherHeaders } : undefined)
      .then(r => r.json())
  ...
  async function submitAnswers() {
    ...
    try {
      const token = localStorage.getItem('token')
      const isPendingStudent = Boolean(pendingRegistration) || !token
      const payload = {
        edu_level: eduLevel,
        major,
      ...
      if (!isPendingStudent && token) headers.Authorization = `Bearer ${token}`

      const res = await fetch(isPendingStudent ? '/api/onboarding/preview' : '/api/onboarding/submit', {
        method: 'POST',
        headers,
      ...
      if (res.ok) {
        ...
        updatePendingRegistration({ eduLevel, major })
      } else {
        setScore(0)
  ...
  const isPendingRegistration = Boolean(pendingRegistration)

  // ── Step 0 — 保存注册信息，进入选择身份 ──────────────────────────────────

  async function handleRegNext() {
    setRegError('')
    const { realName, school, teacherUserId, phone } = regInfo
    ...
    try {
      if (pendingRegistration) {
        updatePendingRegistration({ profile: regInfo })
      } else {
  ...
  async function handleFinish() {
    ...
    try {
      const res = await fetch('/api/onboarding/register-complete', {
        method: 'POST',
      ...
      const data = await res.json()
      if (!res.ok) {
        setFinishError(data.error || '注册完成失败，请稍后重试')
        return
      }
      ...
      setFinishError(err instanceof Error ? err.message : '注册完成失败，请稍后重试')
    } finally {
      ...
      <div style={{ width: '100%', maxWidth: 720, background: '#fff', borderRadius: 16, boxShadow: '0 8px 32px rgba(24,59,75,0.1)', overflow: 'hidden' }}>
        ...
        {step === 3 && score !== null && (
          <div style={{ padding: '40px 48px' }}>
            ...
            {/* 操作按钮 */}
            {finishError && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#dc2626', fontSize: 13, marginBottom: 12 }}>
                <AlertCircle size={14} />{finishError}
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <button onClick={handleFinish} disabled={finishSaving} style={{
                width: '100%', padding: '13px', borderRadius: 10, border: 'none',
                background: finishSaving ? '#9bb8bd' : '#1d6f78', color: '#fff', fontWeight: 700, fontSize: 15, cursor: finishSaving ? 'wait' : 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              }}>
                {finishSaving ? <Loader2 size={17} style={{ animation: 'spin 1s linear infinite' }} /> : <BookOpen size={17} />}
                {finishSaving ? '正在完成注册…' : isPendingRegistration ? '完成注册并进入系统' : '进入个性化学习'}
                {!finishSaving && <ChevronRight size={18} />}
              </button>
            </div>
          </div>
        )}
