...
import { db } from '@/db'
import { courseAssignments, courseAssignmentSubmissions, courseStudyLogs, learningPlans, moduleScores, trainingProjects, userGameState, users } from '@/db/schema'
...
export async function GET(req: NextRequest) {
  ...
  if (me.teacherUserId) {
    classmates = await db.select().from(users)
      .where(and(eq(users.role, 'student'), eq(users.teacherUserId, me.teacherUserId)))
  } else if (me.className && me.school) {
    classmates = await db.select().from(users)
      .where(and(eq(users.role, 'student'), eq(users.school, me.school), eq(users.className, me.className)))
  } else if (me.school) {
    classmates = await db.select().from(users).where(and(eq(users.role, 'student'), eq(users.school, me.school)))
  } else {
    classmates = await db.select().from(users).where(and(eq(users.role, 'student'), eq(users.orgId, me.orgId)))
  }

  const allScores = await db.select().from(moduleScores)
  const projects = await db.select().from(trainingProjects)
  const plans = await db.select().from(learningPlans).orderBy(desc(learningPlans.createdAt))
  const eduByUser = new Map<string, 'college' | 'undergraduate'>()
  for (const plan of plans) {
    eduByUser.set(plan.userId, plan.eduLevel === 'undergraduate' ? 'undergraduate' : 'college')
  }
  ...
  const assignmentScoresByUser = new Map<string, Map<string, number[]>>()
  for (const row of assignmentRows) {
    if (typeof row.score !== 'number') continue
    const byTraining = assignmentScoresByUser.get(row.userId) ?? new Map<string, number[]>()
    const values = byTraining.get(row.trainingId) ?? []
    values.push(row.score)
    byTraining.set(row.trainingId, values)
    assignmentScoresByUser.set(row.userId, byTraining)
  }
  ...
  const ranking = classmates.map(user => {
    ...
    const assignmentScores = assignmentScoresByUser.get(user.userId) ?? new Map<string, number[]>()
    const eduLevel = eduByUser.get(user.userId) ?? 'college'
    ...
    for (const [trainingId, scores] of assignmentScores.entries()) {
      const average = scores.reduce((sum, score) => sum + score, 0) / scores.length
      totalEarnedHours += scoreToEarnedHours(
        getCourseComponentMaxHours(projects, trainingId, eduLevel, 'assignment'),
        average,
      )
    }
  ...
  ranking.sort((left, right) => {
    if (right.totalEarnedCredits !== left.totalEarnedCredits) return right.totalEarnedCredits - left.totalEarnedCredits
  ...
  return NextResponse.json({
    scope: me.teacherUserId || (me.className && me.school) ? '本班' : me.school ? '本校' : '全平台',
