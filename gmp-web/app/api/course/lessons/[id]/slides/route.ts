...
const POWERPOINT_EXPORT_WIDTH = 2560
const POWERPOINT_EXPORT_HEIGHT = 1440
const PPT_IMAGE_CACHE_DIR = join(process.cwd(), '.next', 'cache', 'ppt-preview-images')
...
function slideImageSortKey(fileName: string) {
  return Number(fileName.match(/(\d+)/)?.[1] ?? 0)
}

async function readExportedPngDataUrls(outputDir: string) {
  const files = (await readdir(outputDir).catch(() => []))
    .filter(file => /\.png$/i.test(file))
    .sort((left, right) => slideImageSortKey(left) - slideImageSortKey(right))

  const images: string[] = []
  for (const file of files) {
    const buffer = await readFile(join(outputDir, file))
    images.push(`data:image/png;base64,${buffer.toString('base64')}`)
  }
  return images
}

async function exportPptxImagesWithPowerPoint(buffer: Buffer) {
  if (process.platform !== 'win32') return []

  const cacheKey = `${createHash('sha1').update(buffer).digest('hex')}-${POWERPOINT_EXPORT_WIDTH}x${POWERPOINT_EXPORT_HEIGHT}`
  const outputDir = join(PPT_IMAGE_CACHE_DIR, cacheKey)
  const cached = await readExportedPngDataUrls(outputDir)
  if (cached.length > 0) return cached

  await mkdir(outputDir, { recursive: true })
  const inputPath = join(outputDir, `source-${randomUUID()}.pptx`)
  await writeFile(inputPath, buffer)

  const script = `
$ErrorActionPreference = 'Stop'
$inputPath = ${powerShellLiteral(inputPath)}
$outputDir = ${powerShellLiteral(outputDir)}
$powerPoint = $null
$presentation = $null
try {
  $powerPoint = New-Object -ComObject PowerPoint.Application
  $presentation = $powerPoint.Presentations.Open($inputPath, $true, $false, $false)
  $presentation.Export($outputDir, 'PNG', ${POWERPOINT_EXPORT_WIDTH}, ${POWERPOINT_EXPORT_HEIGHT})
} finally {
...
`
  ...
  try {
    ...
    await execFileAsync('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-EncodedCommand', encodedCommand], {
      timeout: 90_000,
      windowsHide: true,
      maxBuffer: 1024 * 1024,
    })
    return await readExportedPngDataUrls(outputDir)
  } catch (error) {
    console.warn('[course/lesson/slides] PowerPoint image export failed, falling back to SVG preview:', error instanceof Error ? error.message : error)
...
async function parsePptx(buffer: Buffer): Promise<SlidePreview[]> {
  ...
  const images = await exportPptxImagesWithPowerPoint(buffer)
  if (images.length > 0) {
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!pptUrl) {
    return noStoreJson({ previewType: 'empty', slides: [] })
  }

  const ext = getExt(pptUrl)
  if (ext === '.pdf') {
    return noStoreJson({ previewType: 'pdf', url: pptUrl, slides: [] })
  }
  if (ext !== '.pptx') {
    return noStoreJson({ previewType: 'unsupported', url: pptUrl, slides: [] })
  }
