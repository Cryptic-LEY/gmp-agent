...
export async function POST(req: NextRequest) {
  ...
  for (const provider of providers) {
    ...
    const voice = provider === 'dashscope'
      ? normalizeDashScopeVoice(body.voice)
      : provider === 'edge'
        ? normalizeEdgeVoice(body.voice)
        : normalizeVoice(body.voice)
    ...
    try {
      const result = provider === 'dashscope'
        ? await synthesizeWithDashScope(text, body.voice)
        : provider === 'edge'
          ? await synthesizeWithEdge(text, body.voice)
          : await synthesizeWithOpenAi(text, body.voice)
