...
export type ScenarioQuestionKind = 'single' | 'multiple' | 'case'
...
export function buildProject7BossQuestions(track: EducationTrack, carrier: CarrierCase): ScenarioQuestion[] {
  ...
  const questions: PromptSeed[] = track === 'college'
    ? [
        ...
        ['B03', 'single', '流程执行', '共用检验账号应如何记录？', ['作为数据可靠性偏差报告', '视为便利措施', '忽略'], [0], '行为需可归属。'],
        ...
        ['B07', 'single', '措施执行', '完成 CAPA 的最低证据要求是？', ['批准记录与执行/复核证据', '只培训签到', '只口头通知'], [0], '执行需可审核。'],
    ...
    : [
        ...
        ['B03', 'case', '根因论证', '计算差错与参数超限同时存在，结论应如何建立？', ['区分直接/系统原因', '验证工艺影响', '形成证据链', '任选一项结案'], [0, 1, 2], '根因必须可论证。'],
        ...
        ['B08', 'single', 'CAPA 验证', 'CAPA 可关闭的必要条件是？', ['预定周期内达到接受标准', '完成培训即关闭', '文件已打印'], [0], '有效性决定是否关闭。'],
